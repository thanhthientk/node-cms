...
const getModulePermissions = function () {
    let modules = [];
    for (let module of ALL_MODULES) {
        let pathToIndexFile = path.join(__root, `./admin/${module}/index.js`);
        if (fs.existsSync(pathToIndexFile)) {
            let selfModule = require(pathToIndexFile);
            if (selfModule.permissions) {
                let module = {
                    name: selfModule.info.label,
                    permissions: selfModule.permissions
                };
                modules.push(module);
            }
        }
...
module.exports = {
    ...
    create: function (req, res, next) {
        res.render(_info.views.create, { modules: getModulePermissions() });
    },
    ...
    edit: function(req, res, next) {
        _Module.findById(req.params.id)
            .then(item => {
                res.render(_info.views.create, { item, modules: getModulePermissions() });
            })
            .catch(err => {
                next(err);
            })
    ...
    destroy: function(req, res, next) {
        ...
        if (req.body.listId === '58e2147e5a76660a40aa0206' || req.body.listId.indexOf('58e2147e5a76660a40aa0206') > -1)
            return res.redirect('back');
